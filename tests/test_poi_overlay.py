...
class PoiOverlayTestCase(unittest.TestCase):
    ...
    def test_project_point_supports_anisotropic_pixel_space_projection(self) -> None:
        overlay = PoiOverlay(
            pois_path=self.root / "pois.json",
            map_bounds=(0.0, 0.0, 1.0, 1.0),
            projection_type="pixel_space",
        ...
        self.assertEqual(overlay.project_point(10.0, 20.0, 200, 200), (75, 40))
        self.assertIsNone(overlay.project_point(100.0, 100.0, 200, 200))
    ...
    def test_marker_icon_scales_with_render_scale(self) -> None:
        (self.root / "biligame_points.json").write_text(
            json.dumps(
                [
                    {
        ...
        image_small = np.zeros((120, 120, 3), dtype=np.uint8)
        image_large = np.zeros((120, 120, 3), dtype=np.uint8)

        overlay.render_scale_x = 0.1
        overlay.render_scale_y = 0.1
        overlay._draw_marker(image_small, record, (60, 60))

        overlay.render_scale_x = 0.8
        overlay.render_scale_y = 0.8
        overlay._draw_marker(image_large, record, (60, 60))

        small_pixels = int(np.count_nonzero(image_small[:, :, 2]))
        large_pixels = int(np.count_nonzero(image_large[:, :, 2]))
